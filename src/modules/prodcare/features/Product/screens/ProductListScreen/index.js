...
import { thunkGetChildrenById, thunkGetListProduct } from '../../productSliceV2';
...
import { setPaginationPerPage } from '../../productSlice';
...
function ProductListScreen(props) {
  ...
  const { products, isGettingProductList, pagination, isGettingChildren, rowKey } = useSelector(
    (state) => state.productV2
  );
      ...
      isGettingProductList ? (
        ...
        >
          <Column
            ...
            body={(row) => <span>{row?.data?.order}</span>}
            header={t('STT')}
          ></Column>
          <Column
            style={{ width: '250px' }}
            ...
            body={(row) => (
              <span>
                ...
                {isGettingChildren && rowKey == row?.key ? (
                  <span className="spinner spinner-loader spinner-primary ml-3"></span>
                ) : null}
              </span>
            )}
          ...
          <Column
            style={{ width: '125px' }}
            className="font-weight-bolder border-right"
            body={(row) => <span>{row?.data?.serial}</span>}
            header={t('Serial')}
          ></Column>
          <Column
          ...
          <Column
            className="border-right"
            style={{ width: '50px' }}
            body={(row) => <span>{row?.data?.version}</span>}
          ...
          <Column
            ...
            body={(row) => {
              ...
              let count = row?.data?.product_id
                ? row?.data?.count
                : issues?.filter((is) => is?.data?.status != 'PROCESSED')?.length;
            ...
            body={(row) => {
              return (
                <div className="d-flex flex-row align-items-center justify-content-center gap-2">
                  <KTTooltip text={t('CreateChildComponent')}>
                    <a
                      className="btn btn-icon btn-xs btn-secondary btn-hover-success"
                      onClick={(e) => {
                        e.preventDefault();
                        handleCreateChildComponent(row?.data);
                      }}
                    >
                      <i className="fa-regular fa-octagon-plus p-0 icon-1x" />
                    </a>
                  </KTTooltip>

                  <KTTooltip text={t('Edit')}>
                    <a
                      className="btn btn-icon btn-xs btn-secondary btn-hover-primary"
                      onClick={(e) => {
                        e.preventDefault();
                  ...
                  <KTTooltip text={t('Delete')}>
                    <a
                      className="btn btn-icon btn-xs btn-secondary btn-hover-danger"
                      onClick={(e) => {
                        e.preventDefault();
                        if (row?.hasOwnProperty('product_id')) {
  ...
  function handleRowExpanded(row) {
    if (row.data.product_id && row.children.length <= 1)
      dispatch(
        thunkGetChildrenById({
          componentId: row.data.id,
  ...
  function handleCreateChildComponent(parent) {
    console.log(parent);

    if (parent.product_id) {
      setSelectedComponentItem(parent);
      setSelectedProductItem(products.find((item) => item.data.id == parent.product_id));
    } else {
      setSelectedProductItem(parent);
    }
    setModalEditComponentShowing(true);
  }
  ...
  return (
    <div>
      ...
      <ModalEditComponent
        parentId={selectedComponentItem?.id}
        productId={selectedProductItem?.id}
        level={Number(selectedComponentItem?.level) + 1}
